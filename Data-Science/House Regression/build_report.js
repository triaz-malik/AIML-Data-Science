// Generate House_Prices_Report.docx
// Run with:
...
const OUT = path.resolve(__dirname, "House_Prices_Report.docx");
...
function img(filename, w = 480) {
  ...
  // Read PNG to get dimensions for aspect ratio
  // Fallback: use 480 x ratio. Default ratio 1:0.6 if can't read.
  return new Paragraph({
...
children.push(p("The full preprocessing pipeline, trained base models, and Ridge meta-learner are serialized to artifacts/model.pkl (8.4 MB). A Streamlit app (streamlit_app.py) loads these and exposes an interactive form."));
...
python house_prices.py             # train (writes artifacts/model.pkl)
...
children.push(code(`house-prices/
  house_prices.py          # full pipeline
  streamlit_app.py         # interactive estimator
  README.md                # GitHub project page
  requirements.txt         # pinned dependencies
  .gitignore
  train.csv / test.csv     # competition data
  data_description.txt     # field reference
  figures/                 # 11 generated PNG plots
  artifacts/model.pkl      # serialized models for Streamlit
